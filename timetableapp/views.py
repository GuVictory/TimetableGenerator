...
def home(request):
    ...
    context = {
        ...
        'auth': request.user.is_authenticated, 'user': request.user,
    }
...
def CourseView(request):
    ...
    if request.method == 'POST':
        ...
        if course.is_valid():
            messages.success(request, 'Курс был успешно добавлен.')
            course.save()
        else:
            messages.success(
                request, 'Такой курс уже существует или вы введи не верные значения!')
